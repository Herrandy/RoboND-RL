...
// this is the tube on the ground which we want to hit
...
// onCollisionMsg
    void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
    {
        ...
        {
            // if we collide with the ground, continue.
            ...
            /* TASK 1
            // If we collided with the robot arm and the tube, end episode immediately and add reward
...
// upon recieving a new frame, update the AI agent
	...
	*/
	...
	float sign = (action % 2 == 0) ? 1.0f : -1.0f;
	float velocity = vel[action/2] + actionVelDelta * sign;
	...
	for( uint32_t n=0; n < DOF; n++ )
	{
        ...
        */
		float sign = (action % 2 == 0) ? 1.0f : -1.0f;
        float joint = ref[action/2] + actionJointDelta * sign;
...
// called by the world update start event
    void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
    {
            ...
            */
            ...
            else
            {
                ...
                if( episodeFrames > 1 )
                {
                    ...
                    const float movingAverage  = 0.1f;
		    		// const float movingAverage = 0.0;	
                    // compute the smoothed moving average of the delta of the distance to the goal
		    		// std::cout << "avgGoalDelta: " << avgGoalDelta << std::endl;
                    avgGoalDelta  = (avgGoalDelta * movingAverage) + (distDelta * (1.0f - movingAverage));
        ...
        if( newReward && agent != NULL )
        {
            ...
            if ( episodeFrames % 10 == 0)
                printf("Current Episode: %i\n",episodeFrames);
